...
export const useReservationStore = defineStore("reservation", () => {
  ...
  const getReservationByDate = (date, id) => {
    axios
      .get(`${REST_API_RESERVATION}/${date}/${id}`, {
        headers: {
          "access-token": accessToken,
        },
      })
  ...
  const getAllReservations = () => {
    axios
      .get(`${REST_API_RESERVATION}/now/${loginUserId}`, {
        headers: {
          "access-token": accessToken,
        },
      })
  ...
  const makeReservation = (reservationInfo) => {
    if (
      reservationInfo.payment === 2 &&
      userStore.loginUserInfo.coin < reservationInfo.coin
    ) {
    ...
    axios
      .post(`${REST_API_RESERVATION}`, reservationInfo, {
        headers: {
          "access-token": accessToken,
        },
      })
